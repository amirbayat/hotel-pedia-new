...
export function HotelListing() {
  ...
  const sortBy = (searchParams.get('sort_by') as HotelSortBy | null) ?? 'default'
  ...
  const hasActiveFilters =
    filters.name !== '' || filters.discountedOnly || filters.stars.length > 0 || filters.minPrice != null || filters.maxPrice != null

  const { data, isLoading, isFetching, isError, refetch } = useHotelSearch({ city, checkIn, checkOut, sortBy, page })
  ...
  function handleDestinationSelect(destination: Destination) {
    const citySlug = destination.type === 'city' ? destination.slug : destination.citySlug
        ...
        <div className={styles.body}>
          <div className={styles.list}>
            {isLoading ? (
              ...
              <p className={styles.stateMessage}>هتلی با این مشخصات یافت نشد.</p>
            ) : (
              <div className={[styles.cards, isFetching && styles.cardsFetching].filter(Boolean).join(' ')}>
          ...
          {filtersOpen && <FilterSidebar className={styles.sidebar} value={filters} onChange={handleFiltersChange} />}
        </div>
      </div>
