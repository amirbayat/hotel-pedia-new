  ...
  return (
    <div className={styles.toolbar}>
      <span className={styles.resultCount}>
        {resultCount} هتل و اقامتگاه در {cityLabel}
      </span>
        ...
        <div className={styles.pills}>
          {SORT_PILLS.map((pill) => (
            <button
              key={pill.value}
              type="button"
              className={[styles.pill, pill.value === sortBy && styles.pillActive].filter(Boolean).join(' ')}
      ...
      <div className={styles.filterActions}>
        <button type="button" className={styles.clearButton} onClick={onClearFilters} disabled={!hasActiveFilters}>
          <span>حذف فیلتر</span>
          <IconClose width={24} height={24} aria-hidden />
        </button>

        <button type="button" className={styles.filtersToggle} onClick={onToggleFilters} aria-pressed={filtersOpen}>
          <span>فیلترها</span>
          <IconSwapDown width={24} height={24} aria-hidden />
        </button>
      </div>
